...
class _PathParents(pathlib._PathParents if sys.version_info < (3, 12) else object):
...
class StreamFlowPath(PurePath, ABC):
    ...
    @classinstancemethod
    def _from_parts(self, args, init=sys.version_info == (3, 9)) -> StreamFlowPath:
        if sys.version_info >= (3, 12):
    ...
    @classinstancemethod
    def _from_parsed_parts(self, drv, root, parts, init=sys.version_info == (3, 9)):
        if sys.version_info >= (3, 12):
            return super()._from_parsed_parts(drv, root, parts)
        else:
            self = (
                object.__new__(self)
                if isinstance(self, type)
    ...
    @property
    def parents(self):
        return _PathParents(self) if sys.version_info < (3, 12) else super().parents
...
class LocalStreamFlowPath(
    WindowsPath if os.name == "nt" else PosixPath, StreamFlowPath
):
    def __init__(
        self,
        *args,
    ...
    async def write_text(
        ...
        return cast(Path, super()).write_text(
