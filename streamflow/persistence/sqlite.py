...
class SqliteDatabase(CachedDatabase):
    ...
    async def add_command(self, step_id: int, cmd: str) -> int:
        async with self.connection as db:
            async with db.execute(
                "INSERT INTO command(step, cmd) " "VALUES(:step, :cmd)",
                {"step": step_id, "cmd": cmd},
            ) as cursor:
    ...
    async def list_workflows(self) -> MutableSequence[MutableMapping[str, Any]]:
        async with self.connection as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT name, status, type FROM workflow ORDER BY id DESC"
            ) as cursor:
                return [
                    {"name": row[0], "status": Status(row[1]).name, "type": row[2]}
