...
from typing import IO, TYPE_CHECKING, AnyStr, Literal, Protocol, TypeAlias, cast
...
if TYPE_CHECKING:
    ...
    TarFormat: TypeAlias = Literal[0, 1, 2]
...
async def copyfileobj(src: Stream, dst: Stream, length=None, bufsize=None):
...
async def write(src: Stream, dst: Stream, bufsize: int):
    while bufsize > 0:
        buf = (
...
class BZ2StreamWrapper(CompressionStreamWrapper):
    def __init__(
        self, stream: Stream, mode: Literal["r", "a", "w", "x"], compresslevel: int = 9
    ):
        try:
            import bz2
        except ImportError:
...
class FileStreamReaderWrapper(StreamWrapper):
    def __init__(self, stream: Stream, offset, size, blockinfo=None):
        super().__init__(stream)
        self.size = size
        self.offset = offset
        self.position = 0
    ...
    async def read(self, size: int | None = None) -> AnyStr:
        ...
        if data:
            await cast(SeekableStreamReaderWrapper, self.stream).seek(
...
class AioTarStream:
    ...
    def __init__(
        ...
        self.offset = 0
        self.stream = (
            TellableStreamWrapper(stream)
            if self.mode not in "ra"
            else SeekableStreamReaderWrapper(stream)
        )
        self.index = 0
    ...
    @classmethod
    def open(cls, stream: Stream, mode: Literal["r", "a", "w", "x"] = "r", **kwargs):
    ...
    @classmethod
    def taropen(
        cls, stream: Stream, mode: Literal["r", "a", "w", "x"] = "r", **kwargs
    ) -> Self:
    ...
    async def _find_link_target(self, tarinfo: AioTarInfo):
        if tarinfo.issym():
            linkname = "/".join(
    ...
    async def _getmember(
        self, name, tarinfo: AioTarInfo | None = None, normalize: bool = False
    ) -> AioTarInfo | None:
    ...
    async def addfile(self, tarinfo: AioTarInfo, fileobj=None) -> None:
        self._check("awx")
    ...
    async def getmember(self, name) -> AioTarInfo:
