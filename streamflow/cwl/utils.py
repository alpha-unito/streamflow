...
async def _register_path(
    ...
    if real_path := await sf_path.resolve():
        if real_path != sf_path:
...
async def _process_file_token(
    ...
    is_literal = is_literal_file(token_value, job.name)
...
def is_expression(expression: Any) -> TypeIs[str]:
