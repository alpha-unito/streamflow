    ...
    async def _persist_token(
        ...
        if token.persistent_id:
            raise WorkflowDefinitionException(
                f"Step {self.name} failed to save token: Token already has a persistent id: {token.persistent_id}"
            )
        await token.save(self.workflow.context.database, port_id=port.persistent_id)
        if input_token_ids:
            if any(id_ is None for id_ in input_token_ids):
                raise WorkflowExecutionException(
                    f"Step {self.name} cannot establish provenance: "
                    f"One or more input tokens have not been persisted: {input_token_ids}"
                )
...
class Combinator(ABC):
        ...
        return {
            ...
            "combinators": {
                ...
                for k, v in zip(
                    ...
                    await asyncio.gather(
                        *(
                            asyncio.create_task(comb.save(database))
    ...
    async def load(
        ...
        combinator = await type_._load(row["params"], loading_context)
        ...
        combinator.combinators = dict(
            zip(
                ...
                await asyncio.gather(
                    *(
                        asyncio.create_task(Combinator.load(c, loading_context))
...
class CombinatorStep(BaseStep):
    ...
    async def _load(
        ...
        return cls(
            ...
            combinator=await Combinator.load(
                row["params"]["combinator"], loading_context
            ),
        )
    ...
    async def _save_additional_params(
        ...
        return cast(dict[str, Any], await super()._save_additional_params(database)) | {
            "combinator": await self.combinator.save(database)
        }
...
class DeployStep(BaseStep):
    ...
    async def _load(
        ...
        return cls(
            ...
            deployment_config=await loading_context.load_deployment(
                params["deployment_config"]
            ),
            connector_port=cast(
                ConnectorPort,
                await loading_context.load_port(params["connector_port"]),
            ),
        )
    ...
    async def _save_additional_params(
        ...
        await self.deployment_config.save(database)
        return cast(dict[str, Any], await super()._save_additional_params(database)) | {
        ...
        return cast(dict[str, Any], await super()._save_additional_params(database)) | {
            ...
            "output_processors": {
                ...
                for k, v in zip(
                    ...
                    await asyncio.gather(
                        *(
                            asyncio.create_task(p.save(database))
            ...
            "command": await self.command.save(database) if self.command else None,
        }
...
class GatherStep(BaseStep):
    ...
    async def _load(
        ...
        return cls(
            ...
            workflow=await loading_context.load_workflow(row["workflow"]),
            depth=params["depth"],
            size_port=await loading_context.load_port(params["size_port"]),
        )

    async def _save_additional_params(
        self, database: Database
    ) -> MutableMapping[str, Any]:
        await self.get_size_port().save(database)
        return cast(dict[str, Any], await super()._save_additional_params(database)) | {
            "depth": self.depth,
        ...
        if status != Status.FAILED:
            for key in (k for k in self.token_map.keys() if k not in keys_completed):
                ...
                await self.size_map[key].save(
                    self.workflow.context.database, size_port.persistent_id
                )
...
class InputInjectorStep(BaseStep, ABC):
    ...
    async def _load(
        ...
        return cls(
            ...
            job_port=cast(
                JobPort,
                await loading_context.load_port(row["params"]["job_port"]),
            ),
        )
        ...
        if input_ports:
            while True:
                ...
                if not token.persistent_id:
                    await token.save(
                        self.workflow.context.database,
...
class ScheduleStep(BaseStep):
    ...
    async def _save_additional_params(
        ...
        await self.get_output_port("__job__").save(database)
        params = cast(
            dict[str, Any], await super()._save_additional_params(database)
        ) | {
            "connector_ports": {
            ...
            "binding_config": await self.binding_config.save(database),
...
class ScatterStep(BaseStep):
    ...
    async def _load(
        ...
        return cls(
            name=row["name"],
            workflow=await loading_context.load_workflow(row["workflow"]),
            size_port=await loading_context.load_port(row["params"]["size_port"]),
        )
