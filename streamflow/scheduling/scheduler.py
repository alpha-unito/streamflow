...
class DefaultScheduler(Scheduler):
    ...
    def _allocate_job(self,
        ...
        if logger.isEnabledFor(logging.DEBUG):
            if len(selected_locations) == 1:
                is_local = isinstance(
                    self.context.deployment_manager.get_connector(selected_locations[0].deployment), LocalConnector)
                logger.debug(
                    "Job {name} allocated {location}".format(
                        name=job.name,
                        location=("locally" if is_local else
        ...
        for loc in selected_locations:
            if loc not in self.location_allocations:
                self.location_allocations.setdefault(loc.deployment, {}).setdefault(loc.name, LocationAllocation(
                    name=loc.name, deployment=loc.deployment)).jobs.append(job.name)
    ...
    def _deallocate_job(self, job: str):
        ...
        if logger.isEnabledFor(logging.INFO):
            if len(job_allocation.locations) == 1:
                is_local = isinstance(
                    self.context.deployment_manager.get_connector(job_allocation.locations[0].deployment),
                    LocalConnector)
                logger.info(
                    "Job {name} deallocated {location}".format(
                        name=job,
                        location=("from local location" if is_local else
                                  "from location {loc}".format(loc=job_allocation.locations[0]))))
            else:
                logger.info(
                    "Job {name} deallocated from locations {locations}".format(
                        name=job,
                        locations=', '.join([str(loc) for loc in job_allocation.locations])))

    def _get_binding_filter(self, config: Config):
        if config.name not in self.binding_filter_map:
            self.binding_filter_map[config.name] = binding_filter_classes[config.type](**config.config)
    ...
    async def _get_locations(self,
        ...
        for i in range(locations):
            ...
            if selected_location is not None:
                ...
                available_locations = {k: v for k, v in available_locations.items() if v != selected_location}
            else:
                return None
    ...
    def _is_valid(self,
                  ...
                  hardware_requirement: Hardware) -> bool:
        if location.name in self.location_allocations.get(location.deployment, {}):
            running_jobs = list(
                filter(lambda x: (self.job_allocations[x].status == Status.RUNNING or
                                  self.job_allocations[x].status == Status.FIREABLE),
        ...
        if location.hardware is not None and hardware_requirement is not None:
            used_hardware = sum((self.job_allocations[j].hardware for j in running_jobs),
    ...
    async def _process_target(self,
                              target: Target,
        ...
        async with self.wait_queues[deployment]:
            while True:
                async with job_context.lock:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug("Retreiving available locations for job {} on {}.".format(
                    ...
                    valid_locations = {k: loc for k, loc in available_locations.items() if self._is_valid(
                        location=loc, hardware_requirement=hardware_requirement)}
                    if valid_locations:
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug("Available locations for job {} on {} are {}.".format(
                                job_context.job.name,
                                posixpath.join(deployment, target.service) if target.service else deployment,
                        ...
                        if target.scheduling_group is not None:
                            ...
                            self.allocation_groups[target.scheduling_group].append(job_context.job)
                        ...
                        logger.debug("No location available for job {} on deployment {}.".format(
                            job_context.job.name,
                            posixpath.join(deployment, target.service) if target.service else deployment))
    ...
    async def schedule(self,
        ...
        wait_tasks = [asyncio.create_task(
