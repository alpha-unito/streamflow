...
async def test_workflow(context: StreamFlowContext):
    ...
    loading_context = WorkflowBuilder(new_workflow)
    await loading_context.load_full_workflow(context, workflow.persistent_id)
    ...
    for original_processor, new_processor in zip(
        exec_step.output_processors.values(), new_workflow.steps[exec_step.name].output_processors.values()
    ):
