...
async def test_inject_remote_input(
    context: StreamFlowContext, file_type: str, exists: str
) -> None:
    ...
    assert exists == "literal" or input_tokens[0].value["path"] == str(remote_path)
    ...
    assert exists == "literal" or output_tokens[0].value["path"] == str(remote_path)
