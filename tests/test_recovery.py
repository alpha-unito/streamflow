    ...
    for step in execute_steps:
        ...
        for job_name in map(
            ...
            if error_type == InjectorFailureCommand.FAIL_STOP:
                if token_type != "primitive":
                    if failure_step == "execute" and num_of_failures > 1:
                        ...
                        expected_failures = (
                            expected_failures
                            if is_last_step
                            else expected_failures * num_of_failures
                        )
                    elif (
                        failure_step == "transfer"
                        and num_of_steps > 1
                        and num_of_failures > 1
                    ):
                        # fst step retries = fst step failures + snd step failures + ...
                        # snd step retries = snd step failures + trd step failures + ...
                        if not is_last_step:
                        ...
                        if failure_step == "execute" or num_of_failures > 1:
                            if not is_last_step:
                                expected_failures += 1
                        else:
                            # fst step retries = fst step failure + snd step failure +
